...
# Calculate non-embedding params by skipping wte and lm_head (works correctly even if weights are tied)
num_params_non_embedding = sum(
    p.numel() for name, p in orig_model.named_parameters() if "wte" not in name and "lm_head" not in name
)
# Sanity check: non-embedding params should be close to 12 * n_layer * n_embd^2 (theoretical Transformer params)
theoretical_params = 12 * num_layers * model_dim**2
assert abs(num_params_non_embedding / theoretical_params - 1.0) < 0.02, (
    f"Non-embedding params {num_params_non_embedding:,} differs from theoretical {theoretical_params:,} by more than 2%"
)
print0(f"Number of parameters: {num_params:,} (non-embedding: {num_params_non_embedding:,})")
num_flops_per_token = model.estimate_flops()
print0(f"Estimated FLOPs per token: {num_flops_per_token:e}")

# Calculate number of iterations. Either it is given, or from target flops, or from target data:param ratio (in that order)
# NOTE: For Chinchilla ratio, we use non-embedding params because embedding size depends on vocab size, not model capacity
...
if num_iterations > 0:
    ...
    # calculate the number of iterations from the target param data ratio (using non-embedding params)
...
print0(
    f"Tokens : Params (non-embedding) ratio: {total_batch_size * num_iterations / num_params_non_embedding:.2f}"
)  # Chinchilla is ~20
...
def build_val_loader():
    return tokenizing_distributed_data_loader(device_batch_size, max_seq_len, split="val", device=device)
