            ...
            for k in ["exp_avg", "exp_avg_sq"]:
                if k in state_before[id(p)]:
                    ...
                    if not torch.allclose(before, after, rtol=1e-5, atol=1e-8):
                        ...
                        print0(
                            f"ERROR: {k} changed significantly! max_diff={diff.max().item():.6e}"
                        )
                        print0(
                            f"  before: norm={before.norm().item():.6e}, after: norm={after.norm().item():.6e}"
                        )
...
def process_checkpoint(ctx, ckpt_state, step, baseline_loss, raw_writer, out_dir):
    ...
    for b_idx, B in enumerate(ctx.batch_sizes):
        ...
        losses_for_B = sweep_lrs(
            ctx, ckpt_state, grads, step, B, grad_norm_sq, baseline_loss, raw_writer
        )
    ...
    plot_lr_fits(
        ctx.batch_sizes, ctx.lrs, all_losses, all_coeffs, eps_opts, step, out_dir
    )
...
def measure(cfg: DictConfig):
    ...
    print0(
        f"Run dir: {cfg.run_dir}, depth: {run_cfg.depth}, seq_len: {run_cfg.seq_len}"
    )
    ...
    ckpt_paths = sorted(
        glob.glob(os.path.join(cfg.run_dir, "checkpoints", "step_*.pt"))
    )
    ...
    results_writer.writerow(
        ["step", "eval_loss", "B_noise", "B_noise_r2", "B_simple", "B_simple_r2"]
    )
    ...
    for ckpt_idx, ckpt_path in enumerate(ckpt_paths):
        ...
        print0(f"\n{'=' * 60}")
        print0(
            f"[{ckpt_idx + 1}/{len(ckpt_paths)}] Step {step} | Baseline loss: {baseline_loss:.4f}"
        )
        print0(f"{'=' * 60}")
        ...
        for B, eps_opt in zip(batch_sizes, eps_opts):
            print0(
                f"    B={B:>6}  1/B={1 / B:.2e}  ε_opt={eps_opt:.4f}  1/ε_opt={1 / eps_opt:.2f}"
            )
        ...
        results_writer.writerow(
            [step, step_to_loss[step], B_noise, B_noise_r2, B_simple, B_simple_r2]
        )
        results_file.flush()

        plot_contour(batch_sizes, lrs, all_losses, step, out_dir)
        plot_fits(
            batch_sizes, eps_opts, grad_norm_sqs, B_noise, B_simple, step, out_dir
        )
...
def plot_lr_fits(batch_sizes, lrs, all_losses, all_coeffs, eps_opts, step, out_dir):
    ...
    for i, B in enumerate(batch_sizes):
        ...
        ax.axvline(
            eps_opts[i], color="green", linestyle="--", label=f"ε_opt={eps_opts[i]:.4f}"
        )
...
def plot_fits(batch_sizes, eps_opts, grad_norm_sqs, B_noise, B_simple, step, out_dir):
    ...
    a, b = np.linalg.lstsq(
        np.vstack([np.ones_like(inv_B), inv_B]).T, inv_eps, rcond=None
    )[0]
    ...
    a2, b2 = np.linalg.lstsq(
        np.vstack([np.ones_like(inv_B), inv_B]).T, g_sq, rcond=None
    )[0]
